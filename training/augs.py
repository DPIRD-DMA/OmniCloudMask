...
class BatchResample(RandTransform):
    ...
    def _select_scale_factor(self) -> float:
        """Sample from plateau distribution: linear fade-in, uniform plateau, linear fade-out"""  # noqa
...
class RandomRectangle(RandTransform):
    ...
    def __init__(
        self,
        p: float = 0.1,  # Probability of applying Random Erasing
        ...
        max_count: int = 1,  # Maximum number of erasing blocks per image, area per box is scaled by count # noqa
...
class DynamicZScoreNormalize(DisplayedTransform):
    ...
    def __init__(self, no_data_value: float = 0.0):
        ...
        self.epsilon = 1e-8  # Store as instance variable

    def encodes(self, x: TensorImage):
        # Single mask computation
        mask = x != self.no_data_value

        # More efficient statistics computation
        valid_pixels = mask.sum(dim=(2, 3), keepdim=True)  # Keep dims for broadcasting
        mean = (x * mask).sum(dim=(2, 3), keepdim=True) / valid_pixels

        # Efficient std computation
        diff_sq = (x - mean) ** 2 * mask
        std = torch.sqrt(
            diff_sq.sum(dim=(2, 3), keepdim=True) / valid_pixels + self.epsilon
        )
